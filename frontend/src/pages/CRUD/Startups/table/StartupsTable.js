...
import actionsStartups from 'actions/startups/startupsFormActions';
...
const StartupsTable = () => {
  ...
  const update = (id, data) => {
    dispatch(actionsStartups.doUpdate(id, data));
  };
  ...
  const sendStartupsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('startups/bulk-import', data, {
        headers: {
  ...
  const getStartupsCSV = async () => {
    const response = await axios({
      url: '/startups?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
  ...
  return (
    <div>
      <Widget title={humanize('Startups')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/startups/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Startups'
                  : 'http://localhost:8080/api-docs/#/Startups'
              }
            >
              API documentation for startups
            </LinkMaterial>
          </div>
        )}
      </Widget>
