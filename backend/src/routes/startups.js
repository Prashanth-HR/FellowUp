 ...
 * tags:
 *   name: Startups
 *   description: The Startups managing API
 */
 ...
 *                  type: object
 *                  $ref: "#/components/schemas/Startups"
 *      responses:
 *        200:
...
router.post(
  '/bulk-import',
  wrapAsync(async (req, res) => {
    await StartupsService.bulkImport(req, res, true, req.headers.referer);
    const payload = true;
    res.status(200).send(payload);
  }),
);
...
router.put(
  '/:id',
  wrapAsync(async (req, res) => {
    await StartupsService.update(req.body.data, req.body.id, req.currentUser);
    const payload = true;
    res.status(200).send(payload);
  }),
);
...
router.delete(
  '/:id',
  wrapAsync(async (req, res) => {
    await StartupsService.remove(req.params.id, req.currentUser);
    const payload = true;
    res.status(200).send(payload);
  }),
);
...
router.get(
  '/count',
  wrapAsync(async (req, res) => {
    const payload = await StartupsDBApi.findAll(
      req.query,
...
/**
 *  @swagger
 *  /api/startups/autocomplete:
 *    get:
 *      security:
 *        - bearerAuth: []
 *      tags: [Startups]
 *      summary: Find all startups that match search criteria
 *      description: Find all startups that match search criteria
 *      responses:
 *        200:
...
router.get('/autocomplete', async (req, res) => {
  const payload = await StartupsDBApi.findAllAutocomplete(
    req.query.query,
    req.query.limit,
  );
